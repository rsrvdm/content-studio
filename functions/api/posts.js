...
export async function onRequestPost({ request, env }) {
  const { platform, caption, image = null, scheduled_at, brand_id = null } = await request.json();
  if (!platform || !caption?.trim() || !scheduled_at) {
    return json({ error: "A post needs a platform, a caption and a date." }, 400);
  }
  const row = await env.DB.prepare(
    "INSERT INTO posts (platform, caption, image, scheduled_at, brand_id) VALUES (?, ?, ?, ?, ?) RETURNING *"
  ).bind(platform, caption, image, scheduled_at, brand_id).first();
